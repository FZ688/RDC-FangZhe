    ...
    public static void main(String[] args) {
        ...
        * */
        ...
        for (int i = 0; i < 14; i++) {
            ...
            if (statistic.get(foods.get(index)) == null){
                statistic.put(foods.get(index),1);
                oneWeekFood.add(foods.get(index));
            }else {
                //一周内吃重复食物的次数不能超过5次
                // 价格超过30元的所有食物的次数不能超过3次
                while (statistic.get(foods.get(index)) >= 5 ||
                        (statistic.containsKey(foods.get(1))&&statistic.containsKey(foods.get(3))&&foods.get(index).getPrice() > 30 && statistic.get(foods.get(1))+statistic.get(foods.get(3))>3)){
                    index = r.nextInt(foods.size());
                }
                statistic.put(foods.get(index),statistic.get(foods.get(index))+1);
                oneWeekFood.add(foods.get(index));
            }
        }
